...
namespace CosmicLexicon.Foundation.Introspection.Modules
{
    ...
    public static class TypeExtensions
    {
        ...
        extension(Type type)
        {
            ...
            public Type[] GetAggregatedConstructorParamTypes()
            {
            ...
            public bool ShouldGenericEquals() =>
                //return type.ContainsGenericParameters;
                type.GenericTypeArguments.Length < 1;
            public bool GuidEqual(Guid otherTypeGuid)
                 => type.GUID.CompareTo(otherTypeGuid) == 0;
            public bool EqualOrHasInterfaces<TInterface>()
                => type.EqualOrHasInterfaces<TInterface>(InterfaceFilter.None, typeof(TInterface).ShouldGenericEquals());
            public bool EqualOrHasInterfaces<TInterface>(InterfaceFilter interfaceFilter = InterfaceFilter.None, bool? checkGenricDefenitionEqual = null)
              => type.EqualOrHasInterfaces(typeof(TInterface), interfaceFilter, checkGenricDefenitionEqual);
            public bool HasInterface<T>(InterfaceFilter interfaceFilter = InterfaceFilter.None, bool? checkGenricDefenitionEqual = null)
        where T : class
    => type.HasInterface(typeof(T), interfaceFilter, checkGenricDefenitionEqual);
        ...
        => type.HasInterface<T>(InterfaceFilter.None, typeof(T).ShouldGenericEquals());
            public bool EqualOrHasInterfaces(Type iface, InterfaceFilter interfaceFilter = InterfaceFilter.None, bool? checkGenricDefenitionEqual = null)
            {
            ...
            public bool TryGetPublicParameterlessConstuctor([NotNullWhen(true)] out ConstructorInfo? publicParameterlessConstuctor)
            {
            ...
            public bool HasPublicParameterlessConstuctor() => type.TryGetPublicParameterlessConstuctor(out _);

            public bool HasInterface(Type iface, InterfaceFilter interfaceFilter, bool? checkGenricDefenitionEqual = null) => type.GetInterfaces(interfaceFilter).Any(x => x.GenericEquals(iface, checkGenricDefenitionEqual ?? iface.ShouldGenericEquals()));
            public bool GuidEqual(Type otherType)
            {
                ArgumentNullException.ThrowIfNull(type);
            ...
            public bool HasBaseType() => type.BaseType is { } && type.BaseType != typeof(object);

            public bool EqualOrHasBaseType(Type baseType, bool checkGenricDefenitionEqual)
            {
                if (type.GenericEquals(baseType, checkGenricDefenitionEqual))
                    return true;
                return type.HasBaseType(baseType, checkGenricDefenitionEqual);
            }
            public bool HasBaseType(Type baseType, bool checkGenricDefenitionEquale)
            {
            ...
            public bool GenericEquals(Type other, bool checkGenricDefenitionEqual) => checkGenricDefenitionEqual ? type.GetTypeDefenitionIfItIsGeneric() == other.GetTypeDefenitionIfItIsGeneric()
                    : type == other;
            public bool GenericEqualsOrHasBaseType<TType, TBaseType>(bool checkGenricDefenitionEqual)
        where TType : Type => typeof(TType).GenericEqualsOrHasBaseType(typeof(TBaseType), checkGenricDefenitionEqual);
            public bool GenericEqualsOrHasBaseType(Type baseType, bool checkGenricDefenitionEqual) => type.GenericEquals(baseType, checkGenricDefenitionEqual)
                        || type.HasBaseType(baseType, checkGenricDefenitionEqual);
            public bool GenericEqualsOrHasInterface<TInterfaceType>(InterfaceFilter interfaceFilter = InterfaceFilter.None, bool? checkGenricDefenitionEqual = null)
                where TInterfaceType : Type => type.GenericEqualsOrHasInterface(typeof(TInterfaceType), interfaceFilter, checkGenricDefenitionEqual);
            public bool GenericEqualsOrHasInterface(Type iface, InterfaceFilter interfaceFilter = InterfaceFilter.None, bool? checkGenricDefenitionEqual = null) => type.GenericEquals(iface, checkGenricDefenitionEqual ?? iface.ShouldGenericEquals())
                        || type.HasInterface(iface, interfaceFilter, checkGenricDefenitionEqual);
            public bool TryGetCustomAttribute<T>(out IEnumerable<T> attributes)
        where T : Attribute
            {
                ArgumentNullException.ThrowIfNull(type);
            ...
            //     return true;
            // }
            public bool TryGetParent(Type targetInterface, [NotNullWhen(true)] out Type? parent)
            {
            ...
            public Type[] GetInterfaces(InterfaceFilter interfaceFilter, Type? targetInterface = null)
            {
            ...
            public IReadOnlyList<Type> GetBaseClasses()
            {
                ArgumentNullException.ThrowIfNull(type);
     ...
     where TAttribute : Attribute
        ...
        extension(Type? type)
        {
            ...
            public TypeCode GetTypeCode(bool throwIfUnknown = false)
            {
                if (type is null)
